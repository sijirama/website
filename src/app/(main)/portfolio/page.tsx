...
export default function Page() {
    ...
    return (
        <main className={` ${manrope.className} flex min-h-screen flex-col gap-4 items-center mx-auto pt-2 w-full md:4/5 lg:w-4/6 xl:w-3/6 text-zinc-950  dark:text-zinc-100 -tracking-widest p-2.5  md:p-5`}>
            <div className="flex justify-between items-center w-full">
                <div className="w-full md:w-3/5 flex flex-col gap-2">
                    <p className="font-extrabold -tracking-wide text-lg md:text-xl">
                    ...
                    <div className="my-4 md:my-3" key={i}>
                        <div className="flex items-center justify-between">
                            <div className="flex-col-reverse justify-start flex items-start md:flex-row md:items-center gap-2 md:gap-3 my-2 md:my-1 ">
                                ...
                                <div className="text-xs bg-zinc-900 dark:bg-zinc-200 text-zinc-100 dark:text-black rounded-md py-0.5 px-1 md:px-3 font-semibold tracking-wide" >{exp.mode}</div>
                            </div>
                ...
                <div className="items-center gap-3 my-2 grid grid-cols-2 md:grid-cols-3">
                    {projects.map((project, i) => (
                        <a href={project?.link} key={i} target="_blank">
                            <div className="p-2 md:p-3 h-fit hover:bg-zinc-200 dark:hover:bg-zinc-950 transition-colors duration-200 border border-zinc-400 dark:border-zinc-800 rounded-xl flex flex-col justify-between gap-7">
                                <div className="space-y-2">
                                    <p className="text-sm md:text-base font-bold tracking-[0.010em]">{project.title}</p>
                                    <p className="text-xs md:text-sm tracking-wide text-zinc-900 dark:text-zinc-100">{project.description}</p>
                                </div>
